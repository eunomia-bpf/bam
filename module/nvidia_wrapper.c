...
// Track how symbols were obtained
static bool symbols_from_symbol_get = false;
...
int init_nvidia_symbols(void)
{
    ...
    if (!nv_p2p_get_pages || !nv_p2p_put_pages || !nv_p2p_free_page_table ||
        ...
        // Clean up any partial symbols only if they came from symbol_get
        if (symbols_from_symbol_get) {
...
{
    // Only use symbol_put if symbols were obtained via symbol_get
    if (symbols_from_symbol_get) {
        if (nv_p2p_get_pages) {
    ...
    // If symbols were obtained via kallsyms_lookup_name, we just clear them
    // No need to call symbol_put as we didn't increment reference counts
    ...
    printk(KERN_INFO "NVIDIA P2P symbols cleaned up\n");
}
