...
inline __device__
uint32_t cq_poll(nvm_queue_t* cq, uint16_t search_cid, uint32_t* loc_ = NULL, uint32_t* cq_head = NULL, uint32_t* status = NULL, uint32_t* res0 = NULL) {
    uint64_t j = 0;
    unsigned int ns = 8;
...
//            if ((cid == search_cid) && (phase == search_phase) && (cq->head_mark[loc].load(simt::memory_order_acquire) == UNLOCKED)){
            if ((cid == search_cid) && (phase == search_phase)){
                ...
                if (loc_)
                    *loc_ = cur_head;
                if (status)
                    *status = (cpl_entry & 0xfffe0000) >> 17;
                if (res0)
